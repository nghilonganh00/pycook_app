...
class AddPage(MDScreen):
    ...
    def submit_food(self):
        ...
        foodImage = converImageToBase64(
            app.manager.get_screen("addpage").ids.file_chooser_box.added_image_path
        )
        ...
        recipes = [
            children.ids["ingredient_input"].text
            for children in self.ids["ingredient_input_list"].children
        ]
        makings = [
            {"content": children.ids["making_input"].text, "image": ""}
    ...
    def add_ingredient_input(self):
        ...
        ingredient_input_list.add_widget(
            IngredientInput(ingredientId=self.ingredientTotal)
        )
